...
public class HomeController {
	...
	public String phoneReviewsHome(HttpServletResponse response, ModelMap model) throws IOException{
	...
	public ModelAndView getPhoneReviews(@RequestBody SmartPhone smartPhone, HttpServletResponse response, ModelMap model) throws IOException{
		
		List<YoutubeVideoReview> reviews = reviewService.getReviewDetails(smartPhone.getDisplayName(), 10);
	...
	public ModelAndView syncdata(HttpServletResponse response, ModelMap model) throws IOException{
		...
		for (SmartPhone device : smartPhoneList) {
			reviews = reviewService.retriveReviewDetailsFromYoutube(device.getDisplayName(), 50);
	...
	public ModelAndView deletereview(@RequestBody YoutubeVideoReview review, HttpServletResponse response, ModelMap model) throws IOException{
		
		boolean result = reviewService.deleteReview(review.getVideoId());
		
		if(result){
			List<YoutubeVideoReview> reviews = reviewService.getReviewDetails(review.getDeviceName(), 10);
